% MAIN_circleModel.m
%
% This script derives the spline equations that are used for the foot model
%
%
...
nGrid = [2,15,35,15,2,18];
color = lines(14);
thOffset = -pi/2;
for i=1:6
    thTmp = linspace(Th(i),Th(i+1),nGrid(i));
    rTmp = pwPoly5(Th,P,thTmp);
    [xTmp, yTmp] = pol2cart(thTmp+thOffset,rTmp);
    plot(xTmp,yTmp,'Color',color(i+2,:),'LineWidth',3);  % Segments  
end
[xTmp, yTmp] = pol2cart(Th+thOffset,P(1,:)); 
plot(xTmp,yTmp,'ko','MarkerSize',12,'LineWidth',3); % Knot points
plot(0,0,'ko','MarkerSize',15,'LineWidth',4);  %Ankle Joint
axis equal; axis off;
...
for i=1:6
    ...
    rTmp = pwPoly5(Th,P,thTmp);
    [xTmp, yTmp] = pol2cart(thTmp+thOffset,rTmp);
    plot(xTmp,yTmp,'k-','LineWidth',3);  % Segments  
end
[xTmp, yTmp] = pol2cart(Th+thOffset,P(1,:)); 
plot(0,0,'ko','MarkerSize',15,'LineWidth',4);  %Ankle Joint
axis equal; axis off;

% save2pdf('FootSplineDiagram.pdf');  %From file exchange
